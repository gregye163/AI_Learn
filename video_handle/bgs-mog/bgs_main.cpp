...
const string bgtitle = "background";
const string fgtitle = "foreground";
...
static const char *short_options = "m:t:w:h:VHc:";
...
int main(int argc, char **argv)
{
    ...
    enum CAM_TYPE type = CAM_USB; /* HD camera sensor: OV9750 */
    ...
    fcv::moveWindow(wintitle, width, height);
    fcv::namedWindow(bgtitle);
    fcv::moveWindow(bgtitle, width, height);
    fcv::namedWindow(fgtitle);
    fcv::moveWindow(fgtitle, width, height);
    ...
    if (type == CAM_USB)
    {
    	sprintf(v4l2_dev, "/dev/video%d", mipi);
    	sprintf(isp_dev, "/dev/video%d", mipi);
	std::cout << "v4l2_dev:" << v4l2_dev << std::endl;
    }
    else 
    {
    ...
    /* initial parameters -- first readFrame to image */
    if(ret = v4l2.readFrame(V4L2_PIX_FMT_RGB24,image) < 0){
        return ret;
    }
    Mat background(cv::Size(RGA_ALIGN(width, 16), RGA_ALIGN(height, 16)), CV_8UC1);
    Mat foreground(cv::Size(RGA_ALIGN(width, 16), RGA_ALIGN(height, 16)), CV_8UC1);
    Mat grayFrame(cv::Size(RGA_ALIGN(width, 16), RGA_ALIGN(height, 16)), CV_8UC1);
    //bgsMog bgsmog(grayFrame, 5, 36, 0.01, 6.25, 0.2);
    bgsMog bgsmog(grayFrame, 5, 36, 0.1, 6.25, 0.2);
    ...
    while(1) {
        ...
        fcv::cvtColor(image,grayFrame,cv::COLOR_BGR2GRAY);
        gettimeofday(&t0, NULL);
        bgsmog.updateMogImage(grayFrame, background, foreground);
        gettimeofday(&t1, NULL);

        long elapse = ((t1.tv_sec * 1000000 + t1.tv_usec) - (t0.tv_sec * 1000000 + t0.tv_usec)) / 1000;
        cout <<"MOG costs "<< elapse <<" miliseconds"<< endl;
        
        /* Window -- drawing frame */
        gettimeofday(&t0, NULL);
        fcv::imshow(wintitle, image, NULL);    
        // fcv::imshow(foretitle, foreground, NULL);
        Mat fgShow(image.rows, image.cols, CV_8UC3);
        for (int i = 0; i < foreground.rows; i++){
                for (int j = 0; j < foreground.cols; j++){
                     fgShow.at<uchar>(i,3*j) = foreground.at<uchar>(i,j);
                     fgShow.at<uchar>(i,3*j+1) = foreground.at<uchar>(i,j);
                     fgShow.at<uchar>(i,3*j+2) = foreground.at<uchar>(i,j);
            }
        }   	
        fcv::imshow(fgtitle, fgShow, NULL);
        //fcv::imshow(bgtitle, background, NULL); 
        Mat bgShow(image.rows, image.cols, CV_8UC3);
        for (int i = 0; i < background.rows; i++){
                for (int j = 0; j < background.cols; j++){
                     bgShow.at<uchar>(i,3*j) = background.at<uchar>(i,j);
                     bgShow.at<uchar>(i,3*j+1) = background.at<uchar>(i,j);
                     bgShow.at<uchar>(i,3*j+2) = background.at<uchar>(i,j);
            }
        }   
        fcv::imshow(bgtitle, bgShow, NULL);
        gettimeofday(&t1, NULL);
        fcv::waitKey(1);
        elapse = ((t1.tv_sec * 1000000 + t1.tv_usec) - (t0.tv_sec * 1000000 + t0.tv_usec)) / 1000;
